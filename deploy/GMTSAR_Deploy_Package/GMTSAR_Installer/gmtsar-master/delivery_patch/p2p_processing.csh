...
set OMP_NUM_THREADS = 12
  ...
  set xsearch = `grep "^xsearch " $conf | awk '{print $3}'`
  set ysearch = `grep "^ysearch " $conf | awk '{print $3}'`
  set threshold_snr = `grep threshold_snr $conf | awk '{print $3}'`
  set nx_corr = `grep nx_corr $conf | awk '{print $3}'`
  set ny_corr = `grep ny_corr $conf | awk '{print $3}'`
  if ("x$xsearch" == "x") set xsearch = 256
  if ("x$ysearch" == "x") set ysearch = 256
  if ("x$threshold_snr" == "x") set threshold_snr = 20
  if ("x$nx_corr" == "x") set nx_corr = 200
  if ("x$ny_corr" == "x") set ny_corr = 50
  set master = ` echo $2 `
  set aligned =  ` echo $3 `
  echo ""
    ...
    if ($SAT != "S1_TOPS") then
      ...
      if ($skip_master == 0 || $skip_master == 1) then
        ...
        if ($SAT == "ALOS2_SCAN") then
          ...
          xcorr $master.PRM $aligned.PRM -xsearch $xsearch -ysearch $ysearch -nx $nx_corr -ny $ny_corr
          fitoffset.csh 3 3 freq_xcorr.dat $threshold_snr >> $aligned.PRM
        else if ($SAT == "DJ1") then  
        echo "                     ..精细配准 DJ1 (高精度抗噪: xsearch=$xsearch ysearch=$ysearch nx=$nx_corr ny=$ny_corr threshold_snr=$threshold_snr)"
          set OMP_NUM_THREADS = 12
          xcorr2 $master.PRM $aligned.PRM -xsearch $xsearch -ysearch $ysearch -nx $nx_corr -ny $ny_corr -noshift
          filter_offset.csh freq_xcorr.dat  output.txt
          mv output.txt freq_xcorr.dat
          fitoffset.csh 3 3 freq_xcorr.dat $threshold_snr >> $aligned.PRM          
        else if ($SAT == "S1_STRIP") then
          echo "                     ..精细配准 S1_STRIP (高精度抗噪: xsearch=$xsearch ysearch=$ysearch nx=$nx_corr ny=$ny_corr)"
          xcorr $master.PRM $aligned.PRM -xsearch $xsearch -ysearch $ysearch -nx $nx_corr -ny $ny_corr -noshift
          fitoffset.csh 2 2 freq_xcorr.dat $threshold_snr >> $aligned.PRM
        else
          xcorr $master.PRM $aligned.PRM -noshift -xsearch $xsearch -ysearch $ysearch -nx $nx_corr -ny $ny_corr
          fitoffset.csh 2 2  freq_xcorr.dat $threshold_snr >> $aligned.PRM
        endif
        echo "                     ..重采样    "
        echo ""
