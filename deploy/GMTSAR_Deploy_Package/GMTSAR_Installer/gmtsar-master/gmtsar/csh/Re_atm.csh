...
# -El: Linear regression (OLS)
# -i3,2: X=col 3(elev), Y=col 2(phase)
gmt regress tmp_combined.xyz -i3,2 -El -C95 > tmp_reg_result.txt

# 从输出中提取 Slope 和 Intercept
# GMT 6.x 的输出通常在头部的注释行中或者作为单独的行
set intercept = `grep "Intercept" tmp_reg_result.txt | head -1 | awk '{print $NF}'`
set slope = `grep "Slope" tmp_reg_result.txt | head -1 | awk '{print $NF}'`

# 如果 grep 没抓到（比如 GMT 版本差异），尝试另一种解析方式
if ( "$slope" == "" ) then
    set slope = `grep -v "^#" tmp_reg_result.txt | head -1 | awk '{print $2}'`
    set intercept = `grep -v "^#" tmp_reg_result.txt | head -1 | awk '{print $1}'`
endif
