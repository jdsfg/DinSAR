...
echo $LED
if ( ! -f $LED ) then
    # Try to find it in SLC or raw if not in current dir
    if ( -f ../../SLC/$LED ) then
        ln -sf ../../SLC/$LED .
    else if ( -f ../../raw/$LED ) then
        ln -sf ../../raw/$LED .
    else
        echo "Warning: LED file $LED not found locally or in common paths."
        # Keep the original attempt as fallback
        ln -s ../../*/raw/$LED .
    endif
endif
