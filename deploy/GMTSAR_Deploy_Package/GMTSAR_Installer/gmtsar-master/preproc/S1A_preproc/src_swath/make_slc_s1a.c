...
int main(int argc, char **argv) {
	...
	tree *xml_tree;
	//修改轨道数据容量到1000个状态向量
	state_vector sv[1000];
	int ch, n = 0, nc = 0, nlmx = 0;
//程序在参数不足时会直接退出，因此后续访问 argv[3] 是绝对安全的。
	if (argc < 4)
		die(USAGE, "");
...
// -----------------------------------------------------------------------
    // [修复] 频率读取逻辑：适配 Sentinel-1 和 Tianyi (PowerChina-1)
    // -----------------------------------------------------------------------
    
    // 1. 尝试使用 Sentinel-1 的标准路径读取
    search_tree(xml_tree, "/product/generalAnnotation/productInformation/radarFrequency/", tmp_c, 1, 0, 1);

    if (tmp_c != NULL && strlen(tmp_c) > 0) {
        // [情况A] 读取成功 (标准 S1 数据或路径一致)
        prm->lambda = c_speed / str2double(tmp_c);
    } else {
        // [情况B] 路径不匹配，尝试读取 Tianyi 数据
        // 注意：这里不再是“猜测”，而是基于 grep 实测数据的特定适配
        
        // 尝试更短的路径 (有些国产卫星结构较浅)，这是一种探测尝试
        search_tree(xml_tree, "/product/radarFrequency/", tmp_c, 1, 0, 1);
        
        if (tmp_c != NULL && strlen(tmp_c) > 0) {
             prm->lambda = c_speed / str2double(tmp_c);
        } else {
             // [情况C] 彻底读不到标签，使用天仪卫星实测固定值
             fprintf(stderr, "WARNING: <radarFrequency> tag not found via standard paths.\n");
             fprintf(stderr, "ACTION: Applying known frequency for PowerChina-1 (Tianyi).\n");
             fprintf(stderr, "VALUE:  5400000100 Hz (Verified from raw XML)\n");
             
             // 使用刚刚 grep 到的精确数值，而不是模糊值
             double tianyi_freq = 5400000100.0; 
             prm->lambda = c_speed / tianyi_freq;
        }
    }
    // -----------------------------------------------------------------------
/*
 ...
 */
	...
	if (strlen(tmp_c) > 0) {
		...
		prm->SC_clock_start = prm->clock_start; // 默认值
	}
